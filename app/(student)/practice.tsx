      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 90 }} showsVerticalScrollIndicator={false} onScroll={bar.onScroll} scrollEventThrottle={16}>
        ...
        <View onLayout={bar.onHeaderLayout} style={{ paddingTop: insets.top + 16, paddingHorizontal: 22, paddingBottom: 30 }}>
          ...
          <Pressable hitSlop={10} onPress={cycleScope} style={styles.pill}>
            <Text style={{ fontSize: 13, fontWeight: '700', color: PLUM_SOFT, letterSpacing: 0.5 }}>{scopePill} ▾</Text>
          </Pressable>
          <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 8, marginTop: 10 }}>
            <Text className="font-serif" style={{ fontSize: scopeFont, lineHeight: Math.round(scopeFont * 1.1), fontWeight: '800', color: PLUM, letterSpacing: 0.5 }}>{scopeStr}</Text>
            <Text style={{ fontSize: 16, color: '#6a4f9a', fontWeight: '700', marginBottom: 6 }}>遍</Text>
          </View>
        ...
        {isLoading ? (
          ...
          <View style={{ padding: 24, alignItems: 'center' }}>
            <Text style={{ fontSize: 13, color: INK3, textAlign: 'center', lineHeight: 20 }}>加载失败,请检查网络后重试(不代表没有功课)</Text>
          </View>
        ) : vows.length === 0 ? (
          <View style={{ padding: 24, alignItems: 'center' }}>
            <Text style={{ fontSize: 13, color: INK3, textAlign: 'center', lineHeight: 20 }}>还没有功课。加入班级或开始自学后,系统会按专业自动为你发愿。</Text>
          </View>
        ) : (
          <>
            {/* 我的功课(计数型):2 列网格,「添加」圆形加号直接计数(写 practice_logs)。
                观修·座次(时长型)改回独立一行 + 单独小节(PM 2026-07-17 定,撤销同日早些时候的
                "两类统一一列"处理——两类卡片信息形状不同,硬凑同尺寸不如各自排布清楚)。
                管理入口挂在这里但门槛看两类合计:哪怕这次没有计数型功课、只有观修自定功课,
                也不能让「管理」入口跟着计数型网格一起消失(ManageVowsModal 本就管两类的自定功课)。 */}
            {(countVows.length > 0 || durationVows.length > 0) ? (
              <View style={{ paddingHorizontal: 16, marginTop: 30 }}>
                <View style={styles.sechRow}>
                  ...
                  <Pressable hitSlop={8} onPress={() => setManageOpen(true)}><Text style={styles.manageLink}>管理</Text></Pressable>
                </View>
                {countVows.length > 0 ? (
                      ...
                      return (
                        ...
                        <View key={v.vowId} style={[styles.gcard, hit && styles.gcardHit]}>
                          <Pressable style={styles.tx} onPress={() => router.push(`/vow/${v.vowId}`)}>
                            ...
                            <Text numberOfLines={1} style={{ fontSize: 12, marginTop: 3, fontWeight: hit ? '700' : '400', color: hit ? SAGE_DARK : INK3 }}>
                              今日 {v.todayCount.toLocaleString()}{goal != null ? ` / 目标 ${goal.toLocaleString()}` : ''} {v.unit}
                            </Text>
                          </Pressable>
                          <Pressable hitSlop={6} style={styles.dakaCircle} onPress={() => openQc(v.vowId)}><Plus size={16} color={SAFFRON_DARK} /></Pressable>
                        </View>
                      );
                    })}
                  </View>
            ...
            {/* 观修·座次(时长型):独立一行 + 副标题区分,不跟计数型挤同一个网格 */}
            {durationVows.length > 0 ? (
              <Section title="观修" sub="按时长记录 · 达门槛分钟数记 1 座">
                {durationVows.map((v) => (
                  <View key={v.vowId} style={[styles.gcard, { width: '100%' }]}>
                    <Pressable style={styles.tx} onPress={() => router.push(`/vow/${v.vowId}`)}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                        {v.status === 'at_risk' ? <View style={[styles.statusDot, { backgroundColor: CRIM }]} /> : v.status === 'falling_behind' ? <View style={[styles.statusDot, { backgroundColor: WARN }]} /> : null}
                        <Text className="font-serif" numberOfLines={1} style={[styles.gName, { flex: 1 }]}>{v.name}</Text>
                      </View>
                      <Text numberOfLines={1} style={{ fontSize: 12, marginTop: 3, color: INK3 }}>
                  ...
                  {neijiaxing.map((j) => {
                    ...
                    return (
                      <View key={j.vowId} style={styles.jcell}>
                        <Text style={{ fontSize: 13, fontWeight: '700', color: INK }} numberOfLines={1}>{j.name}</Text>
                        ...
                        {expired ? (
                          <Text style={{ fontSize: 11, fontWeight: '700', color: CRIM, marginTop: 4 }}>已过期 · 打卡已锁定</Text>
                        ) : nearExpiry ? (
                          <Text style={{ fontSize: 11, fontWeight: '700', color: WARN, marginTop: 4 }}>剩 {daysLeft} 天到期</Text>
                        ) : null}
                      </View>
                    );
                  })}
        ...
        {!isLoading ? (
          <View style={{ paddingHorizontal: 16, marginTop: 14 }}>
            <Pressable testID={testIds.practice.addVowButton} style={styles.addVowBtn} onPress={() => setAddOpen(true)}>
              <Plus size={16} color={SAFFRON_DARK} /><Text style={{ color: SAFFRON_DARK, fontWeight: '700', fontSize: 14 }}>加功课</Text>
            </Pressable>
          </View>
        ) : null}
      </ScrollView>
...
function Stat({ k, v }: { k: string; v: string }) {
  return (
    <View>
      <Text style={{ fontSize: 12, color: '#6a578f' }}>{k}</Text>
...
function AddVowModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // parseInt 而非 Number(replace(/[^0-9]/g,'')):后者把小数点当"非数字"删掉再拼接(如"2.4"→"24"),
  // parseInt 遇"."正常截断取整数部分(同 vow/[id].tsx 的 PaceSheet/BackfillSheet 一并修·2026-07-17)。
  const targetNum = target.trim() ? parseInt(target, 10) : null;
  const dailyNum = daily.trim() ? parseInt(daily, 10) : null;
      ...
      <Modal visible={open} transparent animationType="slide" onRequestClose={onClose}>
        ...
        <Pressable style={styles.backdrop} onPress={onClose}>
          <Pressable style={[styles.sheet, { maxHeight: '92%' }]} onPress={() => {}}>
            ...
            <Text style={{ fontSize: 12, color: INK3, marginTop: 2, marginBottom: 10 }}>自己发愿一项功课:选修法、定周期,随时计数(随时可改)。</Text>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
                {SOURCES.map((s) => (
                  <Pressable key={s.key} style={[styles.pchip, source === s.key && styles.pchipOn]} onPress={() => pickSource(s.key)}>
                    <Text style={{ fontSize: 13, fontWeight: '600', color: source === s.key ? '#fff' : INK2 }}>{s.label}</Text>
                  </Pressable>
                ))}
              </View>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                {chipPool.map((p) => (
                  <Pressable key={p.id} style={[styles.pchip, pid === p.id && styles.pchipOn]} onPress={() => setPid(p.id)}>
                    <Text style={{ fontSize: 13, fontWeight: '600', color: pid === p.id ? '#fff' : INK2 }}>{p.name}</Text>
                  </Pressable>
                ))}
              </View>
              {isOptional && chipPool.length === 0 ? (
                <Text style={{ fontSize: 12, color: INK3, marginTop: 6 }}>暂无可选项目,请联系管理员配置。</Text>
              ) : null}
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                {PERIODS.map((pr) => (
                  <Pressable key={pr.key} style={[styles.pchip, period === pr.key && styles.pchipOn]} onPress={() => pickPeriod(pr.key)}>
                    <Text style={{ fontSize: 13, fontWeight: '600', color: period === pr.key ? '#fff' : INK2 }}>{pr.label}</Text>
                  </Pressable>
                ))}
              </View>
              <Text style={{ fontSize: 11, color: INK3, marginTop: 6 }}>{PERIODS.find((p) => p.key === period)?.hint}</Text>
              ...
              {limited ? (
                ...
                <>
                  ...
                  <Pressable style={styles.dateField} onPress={() => setCalOpen(true)}>
                    <Text style={{ fontSize: 15, color: endIso ? INK : INK3 }}>{endIso ?? '选择日期'}</Text>
                    <Text style={{ fontSize: 13, fontWeight: '700', color: SAFFRON_DARK }}>选择</Text>
                  </Pressable>
                  {weeklyNeed ? (
                    <View style={styles.paceHint}>
                      <Text style={{ fontSize: 13, color: SAFFRON_DARK, fontWeight: '700' }}>每周约需 {weeklyNeed.toLocaleString()} {unit} · 每天约 {Math.ceil(weeklyNeed / 7).toLocaleString()} {unit}</Text>
                      <Text style={{ fontSize: 11, color: INK3, marginTop: 2 }}>按此进度到 {endIso} 圆满 · 落下会在详情页自动调高</Text>
                    </View>
                  ) : (
                    <Text style={{ fontSize: 11, color: INK3, marginTop: 6 }}>填总目标 + 选截止日后,自动算每周 / 每天需完成多少。</Text>
                  )}
                </>
              ) : (
                ...
                <>
                  ...
                  {dailyNum ? <Text style={{ fontSize: 11, color: SAGE, marginTop: 6 }}>每天念满 {dailyNum.toLocaleString()} {unit} 即达成(每周约 {(dailyNum * 7).toLocaleString()} {unit})</Text> : null}
                </>
              )}
            </ScrollView>
    ...
    <Modal visible={open} transparent animationType="slide" onRequestClose={onClose}>
      <Pressable style={styles.backdrop} onPress={onClose}>
        <Pressable style={styles.sheet} onPress={() => {}}>
          ...
          <Text style={{ fontSize: 12, color: INK3, marginTop: 2, marginBottom: 8 }}>自己发愿的功课可删除;班级 / 自学发放的功课由系统管理。</Text>
          {custom.length === 0 ? (
            <Text style={{ fontSize: 13, color: INK3, textAlign: 'center', paddingVertical: 24 }}>暂无自定功课。点「加功课」自己发愿。</Text>
          ) : (
              ...
              {custom.map((v) => (
                <View key={v.vowId} style={styles.mvRow}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ fontSize: 12, color: INK3, marginTop: 1 }}>已积累 {v.currentCount.toLocaleString()} {v.unit}</Text>
                  </View>
                  {confirmId === v.vowId ? (
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                      <Pressable style={styles.mvConfirm} disabled={del.isPending} onPress={() => del.mutate(v.vowId, { onSuccess: () => setConfirmId(null), onError: (e) => notify('删除失败', (e as Error)?.message ?? '请重试') })}>
                        <Text style={{ fontSize: 12.5, color: '#fff', fontWeight: '700' }}>确认删除</Text>
                      </Pressable>
                      <Pressable hitSlop={6} onPress={() => setConfirmId(null)}><Text style={{ fontSize: 12.5, color: INK3 }}>取消</Text></Pressable>
                    </View>
                  ) : (
                    <Pressable style={styles.mvDel} onPress={() => setConfirmId(v.vowId)}>
                      <Text style={{ fontSize: 12.5, color: '#a13c2e', fontWeight: '700' }}>删除</Text>
                    </Pressable>
                  )}
                </View>
              ))}
          ...
          <Pressable style={[styles.saveBtn, { backgroundColor: 'rgba(43,34,24,0.06)' }]} onPress={onClose}>
            <Text style={{ color: INK2, fontWeight: '700', fontSize: 14 }}>完成</Text>
          </Pressable>
        </Pressable>
      </Pressable>
    </Modal>
