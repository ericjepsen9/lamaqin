...
export function LessonStatusBadge({ st }: { st: LessonStatus | undefined }) {
  if (st === 'complete') return <RNText style={{ fontSize: 10, fontWeight: '700', color: SAGE }}>圆满 ✓</RNText>;
  if (st === 'partial') return <RNText style={{ fontSize: 10, fontWeight: '700', color: SAFFRON_DARK }}>进行中</RNText>;
  return <RNText style={{ fontSize: 10, color: INK3 }}>未学</RNText>;
}
